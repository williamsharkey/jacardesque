...
function nightMarket() {
  ...
  const bass = p.score.addLane(1, 3, new ChannelTile(2, 16), 16);
  ...
  const lead = p.score.addLane(1, 6, new ChannelTile(3, 16), 16);
...
function emptyStation() {
  ...
  const pluck = p.score.addLane(1, 5, new ChannelTile(2, 8), 16);
...
function crowAtDawn() {
  ...
  const snare = p.score.addLane(1, 1, new ChannelTile(1, 16), 16);
  ...
  const crow = p.score.addLane(1, 5, new ChannelTile(3, 8), 8);
...
function tideClock() {
  ...
  const pad = p.score.addLane(1, 5, new ChannelTile(3, 4), 4);
...
function kitchenRadio() {
  ...
  const hat = p.score.addLane(1, 1, new ChannelTile(1, 16), 16);
  for (let i = 0; i < 16; i++) {
    if (i % 2 === 1) fill(hat, i, new NoteTile(N("G#5"), 0.2));
  }

  const kick = p.score.addLane(1, 3, new ChannelTile(3, 16), 16);
  fill(kick, 0, new NoteTile(N("C2")));
  fill(kick, 6, new NoteTile(N("C2")));
  fill(kick, 8, new NoteTile(N("C2")));
  fill(kick, 14, new NoteTile(N("C2")));

  const pluck = p.score.addLane(1, 5, new ChannelTile(2, 16), 16);
...
function glassElevator() {
  ...
  const chords = p.score.addLane(1, 2, new ChannelTile(1, 8), 8);
  ...
  const chime = p.score.addLane(1, 6, new ChannelTile(2, 8), 8);
...
function afterTheSiren() {
  ...
  const drone = p.score.addLane(1, 2, new ChannelTile(1, 2), 4);
  fill(drone, 0, new NoteTile(N("E1"), 4));
  fill(drone, 2, new NoteTile(N("F1"), 4));

  const snare = p.score.addLane(1, 5, new ChannelTile(2, 4), 8);
  fill(snare, 3, new NoteTile(N("D3"), 0.4));
  fill(snare, 7, new ProbGateTile(30), new NoteTile(N("D3"), 0.3));

  const air = p.score.addLane(1, 7, new ChannelTile(3, 2), 4);
...
function bicycleBell() {
  ...
  const bike = p.score.addLane(1, 3, new ChannelTile(2, 16), 16);
  ...
  const bell = p.score.addLane(1, 6, new ChannelTile(3, 8), 8);
...
function snowStatic() {
  ...
  const staticLane = p.score.addLane(1, 1, new ChannelTile(1, 16), 16);
  for (let i = 0; i < 16; i++) {
    fill(staticLane, i, new ProbGateTile(55), new NoteTile(N("C6"), 0.15));
  }

  const pad = p.score.addLane(1, 4, new ChannelTile(2, 4), 4);
  ...
  const kick = p.score.addLane(1, 7, new ChannelTile(3, 8), 8);
  fill(kick, 0, new NoteTile(N("C2"), 1));
  fill(kick, 4, new NoteTile(N("C2"), 1));
  return p;
}
